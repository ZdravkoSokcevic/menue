...
class EditDiscount extends React.Component<IProps, IState>
{
    ...
    componentDidMount(): void {
        this.loadMenuItems();
        this.setState({ selectedMenuItem: this.props.currentItem.menu as TMenu });
        this.setState({ selectedPortion: this.props.currentItem.portion as IPrice});
    }
    ...
    render(): React.ReactNode {
        ...
        const currentInitialValues: IInitialValues = {
            ...
            portion_id: currentItem.portion_id || '',
            value: currentItem.value || '',
            type: currentItem.type || '',
            active_times: currentItem.active_times || 0,
            times: currentItem.times || [],
            time_from: currentItem.time_from || '',
            time_to: currentItem.time_to || '',
            start_at: currentItem.start_at || '',
            end_at: currentItem.end_at || '',
            is_active: currentItem.is_active || false,
        };   

        if(currentItem.times) {
            const arrOfTimeValues: Array<DayOfWeek> = []; 
            let timValues: unknown = currentItem.times;
            let timValuesArr = (timValues as string).split(',');
            timValuesArr.forEach((time: string) => {
                switch(time)
                {
                    case 'mo': arrOfTimeValues.push(DayOfWeek.MONDAY);break;
                    case 'tu': arrOfTimeValues.push(DayOfWeek.TUESDAY);break;
                    case 'we': arrOfTimeValues.push(DayOfWeek.WEDNESDAY);break;
                    case 'th': arrOfTimeValues.push(DayOfWeek.THURSDAY);break;
                    case 'fr': arrOfTimeValues.push(DayOfWeek.FRIDAY);break;
                    case 'sa': arrOfTimeValues.push(DayOfWeek.SATURDAY);break;
                    case 'su': arrOfTimeValues.push(DayOfWeek.SUNDAY);break;
                }
            });
            currentInitialValues.times = arrOfTimeValues;
        }
    ...
    onSubmit = async(event: any) => {
        ...
        const response = await DiscountsAPI.editDiscount(data as IDiscount);
        setTimeout(() => {
