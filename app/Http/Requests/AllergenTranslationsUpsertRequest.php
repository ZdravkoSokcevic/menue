...
class AllergenTranslationsUpsertRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        // 3. Fetch the Allergen being updated from the route parameter (e.g., /extras/{extra})
        ...
        $targetCompanyId = $allergen->company_id;
        if(!$targetCompanyId)
            return false;
        ...
        if ($user->role === User::COMPANY_ADMIN_ROLE || $user->role === User::USER_ROLE) {
            // Must belong to the allergen's current company AND the target company
